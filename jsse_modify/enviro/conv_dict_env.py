...
class JSSE(Env):
    def __init__(self, no_customers, queue_length, no_of_queues, net_flow_out_rate):
        ...
        spaces = {
            'position': gym.spaces.Box(low=0, high=5, shape=(no_of_queues,queue_length)),
            'orientation': gym.spaces.Box(low=0, high=500, shape=(2,))
        }
        ...
        # current state
        self.state = np.zeros(no_customers * no_of_queues + 2)
        #print(self.state)
        #np.append(first, 0)
        #self.state = first
        ...
        self.customers = random.lognormal(-0.35,0.83, no_customers).clip(max=5)
    ...
    def reset(self):
        ...
        #np.append(first, 0)
        self.state = np.zeros(self.no_customers * self.no_of_queues + 2)

        self.queue_counter = np.zeros(self.no_of_queues, dtype=np.int8).tolist()
        self.customers = random.lognormal(-0.35, 0.83, self.no_customers).clip(max=5)
